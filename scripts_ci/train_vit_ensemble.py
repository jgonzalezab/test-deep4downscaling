...
# Set threshold for the precipitation and transform
threshold_pr = 0.1
...
for member_idx in range(n_ensemble_members):
    ...
    # Set model name
    model_name = f'vit_{uncertainty_approach}_member_{member_idx + 1}'
...
# Compute mean and variance of the mixture
mixture_test = deep4downscaling.deep.pred.compute_mixture_bergamma(x_data=x_test_stand, models=ensemble_models,
                                                                   device=device, var_target='pr', mask=y_mask,
                                                                   batch_size=16)
mixture_test.to_netcdf(f'{PREDS_PATH}/vit_{uncertainty_approach}_mixture.nc')

# Compute predictions
pred_test = deep4downscaling.deep.pred.compute_preds_mixture_ber_gamma(x_data=x_test_stand, models=ensemble_models,
                                                                       threshold=threshold_pr, device=device,
                                                                       var_target='pr', mask=y_mask,
                                                                       ensemble_size=10, batch_size=16)
pred_test.to_netcdf(f'{PREDS_PATH}/vit_{uncertainty_approach}.nc')