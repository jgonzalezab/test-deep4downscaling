...
# Create models
model_name = 'vit_atmorep_pr'

reload(deep4downscaling.deep.models)
model = deep4downscaling.deep.models.ViTMultiHead(x_shape=x_train_stand_arr.shape,
...
pred_test = deep4downscaling.deep.pred.compute_preds_multihead(x_data=x_test_stand, model=model,
                                                               ...
                                                               mask=y_mask, batch_size=8)

# Visualize predictions
reload(utils)
utils.evaluate_and_save_plots(target=y_test,
                              pred=pred_test,
                              output_path=f'{FIGURES_PATH}/evaluation_vit_atmorep_pr.pdf',
                              time_to_plot=800,
                              n_members=10)